...
const (
	_                    = iota
	actionWaitingDisband // 等待解散
	actionAgreeDisband   // 同意解散
)
...
type room struct {
	...
	positionUserIDs         map[int]int // key 座位号  value 用户id
	creatorUserID           int         // 创建者userID
	ownerUserID             int         // 房主userID
	number                  string
	desc                    string
...
//房卡计数器
func getOneRoomNumber() string {
	log.Debug("房卡计数器: %v", roomCounter)
