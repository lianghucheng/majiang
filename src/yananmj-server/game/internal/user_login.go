...
//微信登录
func (user *User) weChatLogin(info *data_struct.C2S_WeChatLogin) {
	...
	skeleton.Go(func() {
		...
		if err == mgo.ErrNotFound {
			...
			userData = nil
			user.WriteMsg(&data_struct.S2C_Close{Error: data_struct.S2C_Close_InnerError})
			user.Close()
			return
		}
		...
		if userData.Role == -1 {
			user.WriteMsg(&data_struct.S2C_Close{
				Error:        data_struct.S2C_Close_RoleBlack,
				WeChatNumber: yananConfigData.WeChatNumber,
			})
			user.Close()
			return
		}
		...
		if oldUser, ok := userIDUsers[userData.UserID]; ok {
			...
			oldUser.WriteMsg(&data_struct.S2C_Close{Error: data_struct.S2C_Close_LoginRepeated})
			oldUser.Close()
...
func (user *User) tokenLogin(token string) {
	...
	skeleton.Go(func() {
		...
		if err != nil {
			...
			userData = nil
			user.WriteMsg(&data_struct.S2C_Close{Error: data_struct.S2C_Close_TokenInvalid})
			user.Close()
		}
	}, func() {
		if userData == nil || user.state == userLogout {
...
func (user *User) usernamePasswordLogin(username string, password string) {
	...
	skeleton.Go(func() {
		...
		if err != nil {
			...
			userData = nil
			user.WriteMsg(&data_struct.S2C_Close{Error: data_struct.S2C_Close_UsernameInvalid})
			user.Close()
		}
	}, func() {
		if userData == nil || user.state == userLogout {
...
func (user *User) onLogin(firstLogin bool, anotherLogin bool) {
	...
	user.autoHeartbeat()
	user.WriteMsg(&data_struct.S2C_Login{
		...
		SaleRoomCardNumber: user.data.userData.SaleRoomCardNumber,
		...
		Notice:             yananConfigData.Notice,
		Radio:              yananConfigData.Radio,
		WeChatNumber:       yananConfigData.WeChatNumber,
	})
