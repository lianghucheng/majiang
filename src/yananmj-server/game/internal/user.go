...
var (
	...
	yananPracticeRooms      = make(map[int]interface{}) // key: userID
	yananRoomCardMatchRooms = make(map[int]interface{}) // 房卡匹配场
...
func (user *User) autoHeartbeat() {
	...
	user.WriteMsg(&data_struct.S2C_Heartbeat{})
	//隔120s向客户端发送一次心跳
...
func (user *User) sendTextMessage(r interface{}, message string) {
	...
	broadcast(&data_struct.S2C_TextMessage{
...
func (user *User) sendExpressionMessage(r interface{}, expression int) {
	yananRoom := r.(*YananRoom)
	playerData := yananRoom.userIDPlayerDatas[user.data.userData.UserID]
	broadcast(&data_struct.S2C_ExpressionMessage{
		Position:   playerData.position,
...
func (user *User) completeDailyShare() {
	...
	if ok {
		...
		user.WriteMsg(&data_struct.S2C_CompleteDailyShare{
...
func (user *User) sendGCloudVoiceMessage(r interface{}, fileID string) {
	...
	case *YananRoom:
		yananRoom := r.(*YananRoom)
		playerData := yananRoom.userIDPlayerDatas[user.data.userData.UserID]
		broadcast(&data_struct.S2C_GCloudVoiceMessage{
			Position: playerData.position,
			FileID:   fileID,
		}, yananRoom.positionUserIDs, -1)
	}
...
func (user *User) sendRedPacketMatchOnlineNumber() {
	...
	user.WriteMsg(&data_struct.S2C_UpdateRedPacketMatchOnlineNumber{
...
func (user *User) sendUntakenRedPacketMatchPrizeNumber() {
	...
	skeleton.Go(func() {
		...
		user.WriteMsg(&data_struct.S2C_UpdateUntakenRedPacketMatchPrizeNumber{
			Number: count,
		})
	})
}
...
func (user *User) sendRedPacketMatchRecord(pageNumber int, pageSize int) {
	...
	var items []data_struct.RedPacketMatchRecordItem
	count := 0
	skeleton.Go(func() {
		db := mongoDB.Ref()
		defer mongoDB.UnRef(db)
		...
		for iter.Next(&resultData) {
			items = append(items, data_struct.RedPacketMatchRecordItem{
				ID:            resultData.ID,
		...
		user.WriteMsg(&data_struct.S2C_RedPacketMatchRecord{
			Items:      items,
			Total:      count,
...
func (user *User) takeRedPacketMatchPrize(id bson.ObjectId) {
	if user.data.userData.CircleID < 1 {
		user.WriteMsg(&data_struct.S2C_TakeRedPacketMatchPrize{
			Error: data_struct.S2C_TakeRedPacketMatchPrize_CircleIDInvalid,
		})
	...
	skeleton.Go(func() {
		...
		if resultData == nil {
			user.WriteMsg(&data_struct.S2C_TakeRedPacketMatchPrize{
				Error: data_struct.S2C_TakeRedPacketMatchPrize_IDInvalid,
				ID:    id,
			})
			return
		}
		if resultData.UserID != user.data.userData.UserID || resultData.RedPacket <= 0 {
			user.WriteMsg(&data_struct.S2C_TakeRedPacketMatchPrize{
				Error: data_struct.S2C_TakeRedPacketMatchPrize_NotYetWon,
				ID:    id,
			})
			return
		}
		if resultData.Taken {
			user.WriteMsg(&data_struct.S2C_TakeRedPacketMatchPrize{
				Error:     data_struct.S2C_TakeRedPacketMatchPrize_TakeRepeated,
				ID:        id,
				RedPacket: resultData.RedPacket,
			})
			return
...
func takeRedPacketMatchPrizeSuccess(userID int, id bson.ObjectId, redPacket float64) {
	...
	if user, ok := userIDUsers[userID]; ok {
		user.WriteMsg(&data_struct.S2C_TakeRedPacketMatchPrize{
			Error:     data_struct.S2C_TakeRedPacketMatchPrize_OK,
			ID:        id,
			RedPacket: redPacket,
		})
		cb = func() {
...
func takeRedPacketMatchPrizeFail(userID int, id bson.ObjectId) {
	if user, ok := userIDUsers[userID]; ok {
		user.WriteMsg(&data_struct.S2C_TakeRedPacketMatchPrize{
			Error: data_struct.S2C_TakeRedPacketMatchPrize_Error,
			ID:    id,
		})
	}
