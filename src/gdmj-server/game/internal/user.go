...
var (
	...
	gdroomNumberRooms = make(map[string]interface{})
	hnzzroomNumberRooms = make(map[string]interface{})
	...
	hnzzPracticeRooms      = make(map[int]interface{}) // key: userID----湖南转转
	hnzzRoomCardMatchRooms = make(map[int]interface{}) // 房卡匹配场（前端叫房卡比赛场）----湖南转转
)
