...
func (user *User) transferRoomCard(accountID int, roomCards int) {
	...
	skeleton.Go(func() {
		...
		if otherUser, ok := userIDUsers[otherUserData.UserID]; ok {
			...
			// updateUserData(otherUserData.UserID, bson.M{"$set": bson.M{"roomcards": otherUser.data.RoomCards, "purchasedroomcards": otherUser.data.PurchasedRoomCards}})
		...
		// updateUserData(user.data.UserID, bson.M{"$set": bson.M{"roomcards": user.data.RoomCards, "saleroomcards": user.data.SaleRoomCards}})
